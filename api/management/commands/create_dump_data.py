...
def create_users(created_users=50):
    for _ in range(created_users):
        ...
        if role.role == "Administrator" or role.role == "Teacher":
        ...
        if role.role == "Teacher":
            ...
            t = Teacher.objects.get(teacher=User.objects.get(username=username))
            
            subject_created = create_subject(t, random.choice(subjects))
...
def create_subject(teacher: Teacher, subject_name):
...
def create_course(teacher: Teacher, subject : Subject):
    ...
    return Course.objects.get(teacher=teacher, subject=subject, course_name=subject.subject_name+"->"+str(teacher.teacher.pk))   
